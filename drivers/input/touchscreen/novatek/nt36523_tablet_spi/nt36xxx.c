...
//		ret = ERR_PTR(-ENXIO);
	...
	input_info(true, dev, "%s: Scanoff when cover closed %s\n",
				__func__, platdata->scanoff_cover_close ? "ON" : "OFF");
...
#else
	input_info(true, &ts->client->dev,
			"tc:%d mode:%04X noise:%d lp:%d // v:NO%02X%02X%02X%02X // #%d %d\n",
...
static void nvt_ts_print_coord(struct nvt_ts_data *ts, bool force_release)
{
	...
	for (i = 0; i < TOUCH_MAX_FINGER_NUM; i++) {
		...
		if (ts->coords[i].press && !ts->coords[i].prev_press) {
			...
			input_info(true, &ts->client->dev, "[P] tId:%d.%d x:%d y:%d p:%d major:%d minor:%d loc:%s tc:%d type:%X\n",
...
#else
			input_info(true, &ts->client->dev, "[P] tId:%d.%d p:%d major:%d minor:%d loc:%s tc:%d type:%X\n",
...
#if !IS_ENABLED(CONFIG_SAMSUNG_PRODUCT_SHIP)
				input_info(true, &ts->client->dev, "[C] tId:%d x:%d y:%d p:%d major:%d minor:%d tc:%d type:%X\n",
					i, ts->coords[i].x, ts->coords[i].y, ts->coords[i].palm,
					ts->coords[i].w_major, ts->coords[i].w_minor,
					ts->touch_count, ts->coords[i].status);
#else
				input_info(true, &ts->client->dev, "[C] tId:%d p:%d major:%d minor:%d tc:%d type:%X\n",
					i, ts->coords[i].palm,
					ts->coords[i].w_major, ts->coords[i].w_minor,
					ts->touch_count, ts->coords[i].status);
#endif
...
void nvt_ts_touch_report(struct nvt_ts_data *ts, uint8_t *point_data)
{
	...
	for (i = 0; i < platdata->max_touch_num; i++) {
		...
		if ((status == FINGER_MOVING) || (status == GLOVE_TOUCH) || (status == PALM_TOUCH)) {
			...
			ts->coords[id].palm = (status == PALM_TOUCH) ? 1 : 0;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	// assigned in nvt_parse_dt()
	//ts->platdata->max_touch_num = TOUCH_MAX_FINGER_NUM;

#if TOUCH_KEY_NUM > 0
	// assigned in nvt_get_fw_info()
	//ts->max_button_num = TOUCH_KEY_NUM;
#endif
	...
	input_set_capability(ts->input_dev, EV_KEY, BTN_PALM);
#endif
...
int32_t nvt_ts_suspend(struct device *dev)
{
	...
	if ((ts->lowpower_mode || ts->lcdoff_test) && enter_force_ed_mode == 0) {
		...
		input_info(true, &ts->client->dev, "%s: lp:0x%02X, flip:%d\n",
				__func__, ts->lowpower_mode, ts->flip_enable);
	}
#else
	input_info(true, &ts->client->dev, "%s : lp:%d, test:%d\n",
				__func__, ts->lowpower_mode,
				ts->lcdoff_test);
